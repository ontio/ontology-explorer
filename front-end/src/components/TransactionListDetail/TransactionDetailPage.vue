<template>
  <div class="container container-padding container-margin-top">
    <div class="div-transaction-detail-page form-group pc-display">
      ...
      <!--<div class="row">-->
        <!--<div class="col font-size24 normal_color" style="width: 59rem; height: 6rem; padding: 2rem; line-height: 2rem; background: white">-->
      ...
      <table class="table table-hover">
        ...
        <tr>
          ...
          <td class="td11 table1_item_title td_height font-size24 normal_color" valign="middle" v-else>
            Type: Smart Contract
          </td>
        </tr>
        </tbody>
      ...
      <table class="table table-hover">
        ...
        <tbody>
        <tr>
          <td class="td13 table1_item_title td_height font-size24 normal_color pointer"
      ...
      <table v-if="txflag" class="table table-hover">
        ...
        <tr v-for="tx in Detail.TransferList">
          <td class="td11 td_height" style="background-color:#32A4BE;color:white;padding: 34px 24px;">
            <div class="row ">
              <div class="col-lg-4 padding0-right pointer " @click="toAddressDetailPage(tx.FromAddress)">
                {{tx.FromAddress}}
              </div>
              <div class="col-lg-1 ">sent</div>
              <div class="col-lg-2 ">{{tx.AssetName=='ong'?tx.Amount:toMoney(tx.Amount)}} {{tx.AssetName}}</div>
              <div class="col-lg-1 ">to</div>
              <div class="col-lg-4 padding0-left pointer " @click="toAddressDetailPage(tx.ToAddress)">{{tx.ToAddress}}
              </div>
            </div>
          </td>
        </tr>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getTransactionData() {
        this.$store.dispatch('getTransactionDetailPage', this.$route.params)
      },
      toReturn() {
        this.$router.go(-1)
      },
