...
public class Web3jSdkUtil {
    ...
    private static Web3j web3jSingleton;

    // 单例模式
    public static Web3j getWeb3jSingleton() {
        if (web3jSingleton == null) {
            synchronized (Web3jSdkUtil.class) {
                if (web3jSingleton == null) {
                    web3jSingleton = Web3j.build(new HttpService(paramsConfig.ETH_WEB3J_URL));
                }
            }
        }
...
//        Web3j web3j = getWeb3jSingleton();
        String evm_rpcUrl = "http://172.168.3.73:20339";
        Web3j web3j = Web3j.build(new HttpService(evm_rpcUrl));
        EthTransaction send =  web3j.ethGetTransactionByHash(txHash).send();
        String input = send.getResult().getInput();
        return input;
    }

    public BigDecimal queryErc20Balance(String fromAddress, String contractAddress) {
//        Web3j web3j = getWeb3jSingleton();
        String evm_rpcUrl = "http://172.168.3.73:20339";
        Web3j web3j = Web3j.build(new HttpService(evm_rpcUrl));
        ...
        try {
            ...
            log.error("address:{} get erc20 balance error:{}", fromAddress, e);
        }
        return bigDecimal;
    }
