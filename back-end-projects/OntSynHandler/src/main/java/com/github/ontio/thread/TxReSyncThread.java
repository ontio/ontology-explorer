...
public class TxReSyncThread {
    ...
    @Autowired
    public TxReSyncThread(ParamsConfig paramsConfig, ContractMapper contractMapper, CommonService commonService, Oep8Mapper oep8Mapper, Oep5Mapper oep5Mapper, TransferTransactionPush transferTransactionPush) {
        ...
        try {
            ...
            if (notifyArray.size() == 0) {
                ...
                for (int i = 0, len = notifyArray.size(); i < len; i++) {
                    ...
                    if (ConstantParam.OEP8CONTRACTS.contains(contractAddress)) {
                        //OEP8交易
